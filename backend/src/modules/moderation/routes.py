...
from modules.moderation.domain import ModerationDetails, ModerationSubmission
...
@router.get(
    "/{submission_id}/media/{media_id}/preview",
...
async def media_preview(
    ...
    await context.service.require_permission(context.token, Permission.MODERATION_READ)
    preview = await service.preview(submission_id, media_id)
    return FileResponse(
        preview.path,
...
def _details(details: ModerationDetails) -> SubmissionDetails:
    item = details.submission
        ...
        **_queue_item(item).model_dump(),
        ...
        media=[
            ModerationMedia(
                ...
                width=media.width,
                height=media.height,
                preview_url=(f"/api/v1/admin/submissions/{item.id}/media/{media.id}/preview"),
                caption=media.caption,
                author=media.author,
                approximate_date=media.approximate_date,
