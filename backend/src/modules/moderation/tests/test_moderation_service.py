...
from modules.moderation.domain import ModerationSubmission, PublishAction
...
class FakeRepository:
    def __init__(self, records: list[ModerationSubmission]) -> None:
    ...
    async def get(self, submission_id: UUID, *, lock: bool = False) -> ModerationSubmission | None:
        del lock
        return next((item for item in self.records if item.id == submission_id), None)
...
async def test_claim_changes_pending_once_and_reject_requires_owner() -> None:
    ...
    assert (claimed.status, claimed.version, claimed.claimed_by) == (
...
async def test_revision_records_trimmed_comment_and_releases_claim() -> None:
    ...
    assert revised.status is SubmissionStatus.NEEDS_REVISION
