...
class TimelineEventView(StrictSchema):
    id: UUID
    title: LocalizedText
...
TimelineEventPage = Page[TimelineEventView]


class TimelineRequest(StrictSchema):
    ...
    @field_validator("q", mode="before")
    @classmethod
    def strip_optional_query(cls, value: object) -> object:
        return value.strip() if isinstance(value, str) else value
