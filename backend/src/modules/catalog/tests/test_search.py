...
async def test_postgres_search_returns_first_published_media_preview_as_cover(
    pg_repository: CatalogSearchRepository,
) -> None:
    district_id = await _insert_district(pg_repository)
    entity_id = await _insert_entity(
        pg_repository,
        EntityFixture(district_id, "Обложка объекта", None),
    )
    media_id = uuid4()
    await pg_repository._session.execute(
        text(
            ...
            VALUES (:id,:entity_id,:storage_key,:public_url,:preview_url,'image/webp',640,480,
                    'Подпись','Автор','Архив','published')"""
        ),
        {
            "id": media_id,
            "entity_id": entity_id,
            "storage_key": f"search-test/{media_id}",
            "public_url": f"/api/v1/media/{media_id}/original",
            "preview_url": f"/api/v1/media/{media_id}/preview",
        },
    )

    result = await CatalogSearchService(pg_repository).search(
        replace(_query(), text="обложка объекта", district_id=district_id)
    )

    assert [item.id for item in result.items] == [entity_id]
    assert result.items[0].cover_url == f"/api/v1/media/{media_id}/preview"
