...
class FakeRepository:
    def __init__(self) -> None:
        ...
        self.required_source_ids: set[UUID] = set()
    ...
    async def source_is_required(self, source_id: UUID) -> bool:
        return source_id in self.required_source_ids
...
@pytest.mark.asyncio
async def test_required_published_source_cannot_be_deactivated_or_archived() -> None:
    repository, audit, actor = FakeRepository(), FakeAudit(), uuid4()
    service = AdminCatalogService(lambda: FakeUoW(repository, audit))
    source = await service.create_source(
        AdminSourceCreate(
            title="Единственный источник",
            type=SourceType.ARCHIVE_DOCUMENT,
            author=None,
            publisher=None,
            publication_year=None,
    ...
    repository.required_source_ids.add(source.id)

    with pytest.raises(SourceRequiredError):
        await service.update_source(
            source.id,
            AdminSourcePatch(expected_version=1, is_verified=False),
            actor,
        )
    with pytest.raises(SourceRequiredError):
        await service.archive_source(source.id, expected_version=1, actor_id=actor)

    assert repository.sources[source.id].status is PublicationStatus.PUBLISHED
    assert repository.sources[source.id].is_verified is True
    assert audit.actions == [("catalog.source.create", source.id, 1)]
