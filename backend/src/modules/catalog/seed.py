...
def load_seed_batches(path: Path) -> tuple[SeedPayload, ...]:
    if path.is_file():
        return (load_seed(path),)
    if not path.is_dir():
        raise SeedValidationError("seed path must be a JSON file or an ordered directory")
    entries = sorted(path.iterdir(), key=lambda item: item.name)
    if not entries:
        raise SeedValidationError("seed directory must not be empty")
    if any(
        entry.is_symlink()
        or not entry.is_file()
        or re.fullmatch(r"\d{3}-[a-z0-9-]+\.json", entry.name) is None
        for entry in entries
    ):
        raise SeedValidationError("seed directory contains an unexpected entry")
    batches = tuple(load_seed(entry) for entry in entries)
    _validate_batch_duplicates(batches)
    for batch in batches:
        validate_invariants(batch)
    return batches


def _validate_batch_duplicates(batches: tuple[SeedPayload, ...]) -> None:
    seen: dict[tuple[str, UUID], Record] = {}
    for batch in batches:
        for group in GROUP_ORDER:
            for record in batch.groups[group]:
                key = (group, _uuid(record))
                current = seen.get(key)
                if current is not None and current != record:
                    raise SeedValidationError(
                        f"{group} record {key[1]} conflicts across seed batches"
                    )
                seen[key] = record
...
def validate_invariants(payload: SeedPayload) -> None:
    ...
    _validate_periods(payload.groups["entities"], "entity")
    _validate_periods(payload.groups["relations"], "relation")
...
def _validate_periods(records: tuple[Record, ...], label: str) -> None:
    for record in records:
        period_from = _integer(record, "period_from")
        period_to = _integer(record, "period_to")
        if period_from is not None and period_to is not None and period_from > period_to:
            raise SeedValidationError(f"{label} period_from must not exceed period_to")
...
def _geometry_hex(value: object) -> str | None:
    if value is None:
        return None
    if isinstance(value, str) and value.startswith("SRID=4326;POINT("):
        longitude, latitude = value.removeprefix("SRID=4326;POINT(").removesuffix(")").split()
        return struct.pack("<BIIdd", 1, 0x20000001, 4326, float(longitude), float(latitude)).hex()
    if isinstance(value, str):
        return value.lower()
    return None


def _field_has_drift(group: str, field: str, existing: object, expected: object) -> bool:
    if group == "entities" and field == "coordinate":
        return _geometry_hex(existing) != _geometry_hex(expected)
    return existing != expected


def _has_content_drift(group: str, existing: BaseDBModel, expected: BaseDBModel) -> bool:
    return any(
        _field_has_drift(group, field, getattr(existing, field), getattr(expected, field))
        for field in MODEL_FIELDS[group]
    )
...
class CatalogSeedService:
    ...
    async def seed(self, payload: SeedPayload, *, preserve_existing: bool = False) -> SeedResult:
        created = 0
        unchanged = 0
        preserved = 0
            ...
            for group in GROUP_ORDER:
                ...
                for record in records:
                    ...
                    if current := existing.get(record_id):
                        ...
                        created += 1
        return SeedResult(created=created, unchanged=unchanged, preserved=preserved)
