...
class SqlMediaRepository:
    ...
    async def complete(self, key: UUID, fingerprint: str, record: MediaRecord) -> None:
        async with self._factory.begin() as session:
            ...
            await session.execute(insert(SubmissionMediaModel).values(**_values(record)))
            completed = await session.scalar(
                update(MediaUploadClaimModel)
                .where(
                ...
                .values(media_id=record.id, state="completed")
                .returning(MediaUploadClaimModel.id)
            )
...
def _values(record: MediaRecord) -> dict[str, object]:
    return {
        "id": record.id,
