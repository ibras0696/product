...
The search script lives inside this skill's own directory. In Claude Code, use the provided `CLAUDE_PLUGIN_ROOT`. In Codex or another client where it is unset, resolve the repository root first and set `CLAUDE_PLUGIN_ROOT` to that root; this project exposes the same canonical skill through `.claude/skills/`. Verify that the resulting script path exists before invoking it.
